...
const isValidTikTokUrl = (url) => extractTikTokVideoId(url) !== null;
...
const extractDescriptionFromContext = (pageContent) => {
    ...
    // 1. Try TikTok-specific selectors
    ...
    for (const selector of descriptionSelectors) {
        ...
        if (element.length) {
            ...
            if (text.length > 50) {
                console.log(`✅ Found description via selector "${selector}" (${text.length} chars)`);
                return text;
            }
        }
    }

    // 2. Try JSON-LD or script tags
...
// ---------- Download subtitles ----------
const getTikTokSubtitles = async (videoUrl) => {
    const { execa } = await import('execa');
    const languages = ['en', 'fr', 'es', 'de', 'it', 'pt', 'ru', 'ja', 'ko', 'zh-Hans'];
    let subtitleContent = null;
    for (const lang of languages) {
        const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'tiktok-sub-'));
        try {
            ...
            if (subFile) {
                ...
                console.log(`✅ TikTok subtitles found (${lang})`);
    ...
    console.log(`✅ TikTok subtitles downloaded. Length: ${subtitleContent.length} chars`);
    return { success: true, transcript: subtitleContent, videoTitle: null, duration: null, method: 'subtitles' };
};

// ---------- Download audio ----------
const downloadTikTokAudioWithYtDlp = async (videoUrl) => {
    const { execa } = await import('execa');
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'tiktok-audio-'));
...
const getTikTokAudioTranscript = async (videoUrl) => {
    try {
        console.log("🎬 Attempting audio transcription for TikTok...");
        const audioData = await downloadTikTokAudioWithYtDlp(videoUrl);
...
// ---------- Thumbnail ----------
const getTikTokThumbnail = async (videoUrl) => {
    const { execa } = await import('execa');
    try {
        ...
        // Silently fail – we have other fallbacks
        return null;
    }
};
...
const validateTikTokUrl = (url) => {
    ...
    if (!videoId) return { isValid: false, error: "Invalid TikTok URL format" };
